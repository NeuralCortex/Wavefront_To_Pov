 ...
 * @author pscha
 */
public class PartNormalIndices {

    private final List<String> list;
    private int min = 999999;
    private final int before = 0;

    public PartNormalIndices(List<String> list) {
        this.list = list;
    }

    public String calc(int before) {
        String erg = "normal_indices {\n";
        ...
        for (String line : vertexList) {
            ...
            if (parts[1].split("/").length - 1 > 1) {

                String face1[] = parts[1].split("/");
                String face2[] = parts[2].split("/");
                String face3[] = parts[3].split("/");

                int x = Integer.valueOf(face1[2]);
                int y = Integer.valueOf(face2[2]);
                int z = Integer.valueOf(face3[2]);
